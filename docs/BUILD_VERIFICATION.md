...
| 7   | **Image distribution** (Docker Hub automated build / GHCR / build on host / `docker save`) | `UNRAID_DEPLOYMENT.md` §3                                                                              | Depends on your registry access and preference. A clean-checkout `linux/amd64` build with `--no-cache` was verified here to start, connect to PostgreSQL, and serve; the Docker Hub build itself has not been run.               |
| 7b  | **CPU architecture match**                                                                 | `uname -m` on Unraid, then confirm the pulled image matches                                            | Docker Hub builds `linux/amd64`, which is almost certainly right. This development machine is arm64, so anything built locally would **not** run on an x86_64 host — confirming the match against _your_ host is yours to close. |
